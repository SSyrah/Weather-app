...
// Define todos paths
const todosPath = `${apiPath}/weather`;
...
todos.get(todosPath, checkAccept, async (ctx) => {
  ...
  const orderBy = parseSortQuery({
    urlSortQuery: sort,
    whitelist: ["id", "text", "done"],
  });

  try {
    // const [data] = await pool.execute(`SELECT * FROM todos ${orderBy}`);
     const [data] = await pool.execute 
    (`SELECT * FROM todos WHERE 
    (data -> '$.Wind_dir_MIRC') IS NOT NULL ORDER BY id DESC LIMIT 500;`);
...
// POST /resource
todos.post(todosPath, koaBody, async (ctx) => {
  ...
  try {
    ...
    const [status] = await pool.execute(
      `
          INSERT INTO todos (device_id, data)
